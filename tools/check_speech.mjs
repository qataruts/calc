...
// يحرس: app/js/** app/audio/manifest.json tools/audio_queue.json
//   (كلُّ وحدةٍ تُعلن `SPOKEN`، والبنكُ والقائمةُ يستُرانها)
//
...
/**
 * **همزةُ الوصل لا تُشكَّل في نصٍّ منطوق** — قرارُ المالك (١٣ أغسطس ٢٠٢٦،
 * `METHOD.md §٨`، بلاغُ العائلة `wasl-hamza-and-shared-bank-defects`): «الْمِسْ» لا
 * «اِلْمِسْ» · «اثْنَانْ» لا «اِثْنَانْ».
 *
 * **وسندُه مزدوج**: في الرسم المحقَّق همزةُ الوصل **لا تحمل حركةً أصلاً** (وفي المصحف
 * صادُ الوصل بلا حركة — والحركةُ عليها اصطلاحٌ تعليميّ)، **وفي الأذن** يقرؤها المولّدُ
 * مقطعاً مستقلاً ممطوطاً فتطول الجملةُ ثلثاً بلا لسانٍ عربيّ يقوله (قياسُ اكتب:
 * ٣٫٦٧ ← ٢٫٤٥ث؛ وقياسُنا: «اِثْنَانْ» ١٫٦٦ث ووسيطُ نظائرها ١٫٠٧).
 *
 * **والجردُ بحرفٍ واحد**: الألفُ العارية (`ا`) **لا تحمل حركةً في العربية** إلا أن
 * تكون همزةَ وصلٍ شُكِّلت — فهمزةُ القطع تُكتب بهمزتها (`أ` · `إ` · `آ`)، وألفُ المدّ
 * لا يليها إلا سكونُ ما بعدها. فكلُّ `ا` تليها فتحةٌ أو ضمّةٌ أو كسرة **مخالفةٌ بلا
 * التباس** — والتنوينُ خارجٌ (`شُكْراً` تنوينٌ على ما قبلها لا حركةٌ عليها).
 *
 * **ولا قائمةَ إعفاء**: قرارُ المالك عامٌّ في كل ما يُنطَق، والمعروضُ يتبع المنطوق فلا
 * نصّان لشيءٍ واحد.
 */
export const vowelledWasl = (text) => /ا[َُِ]/.test(String(text));
...
async function main() {
  ...
  /* **والمتقاعدُ خارجَ الباب** (الجلسة ع١): تبديلُ نصٍّ يبدّل مفتاحَه، فيبقى المُصرَّفُ
     القديم في القائمة سجلاً لما سُمع — ولا يُحذَف (حذفُه يمحو أثرَ مراجعةِ الأذن).
     فيُقاعَد بجردٍ (`queue_texts --retire`) ويخرج من هذا الباب، **ويبقى غيرُ المُقاعَد
     بائداً أحمر** فلا يُفتَح البابُ لمن نسي. */
  const stale = queue.filter((row) => !unique.includes(row.text) && !row.retired);
  const retired = queue.filter((row) => row.retired).length;
  ok(stale.length === 0,
    'ولا مدخلَ في القائمة لا تنطقه وحدة (نصٌّ يُولَّد بلا مستهلك)'
    + (retired ? ` — و${retired} مُقاعَداً بجرده` : '')
    + (stale.length ? ` — بائد (يُقاعَد بـ\`queue_texts --retire\` أو يُقلَّم بـ\`--prune\`): `
      + `${stale.slice(0, 5).map((r) => r.text).join(' · ')}` : ''));
  ...
  console.log('\n— همزةُ الوصل: لا تُشكَّل في نصٍّ منطوق (قرارُ المالك · `METHOD.md §٨`) —');
  const wasl = unique.filter(vowelledWasl);
  ok(wasl.length === 0,
    `لا همزةَ وصلٍ مشكولةً في نصٍّ منطوق — بلا إعفاء (${unique.length} نصّاً)`
    + (wasl.length ? ` — **مشكولةٌ في: ${wasl.join(' · ')}**` : ''));
...
function selfTest() {
  ...
  console.log('\n— همزةُ الوصل المشكولة تُمسَك، والقطعُ والمدُّ يمرّان —');
  check(vowelledWasl('اِثْنَانْ') && vowelledWasl('اُنْظُرْ') && vowelledWasl('اِلْمَسْ'),
    'همزةُ الوصل مشكولةً (كسراً وضمّاً) تُمسَك');
  check(!vowelledWasl('اثْنَانْ') && !vowelledWasl('انْظُرْ') && !vowelledWasl('الْمِسْ'),
    'والعاريةُ تمرّ — وهي المطلوبة');
  check(!vowelledWasl('أَرْبَعَةْ') && !vowelledWasl('إِلَى') && !vowelledWasl('آخِرْ'),
    '**وهمزةُ القطع لا تُمَسّ**: تُكتب بهمزتها فتُشكَل كما تُشكَل (أ · إ · آ)');
  check(!vowelledWasl('وَاحِدْ') && !vowelledWasl('سَاخِنْ') && !vowelledWasl('ثَلَاثَةْ'),
    'وألفُ المدّ في وسط الكلمة تمرّ (لا حركةَ عليها أصلاً)');
  check(!vowelledWasl('شُكْرًا') && !vowelledWasl('أَوَّلًا'),
    'والتنوينُ ليس حركةً على الألف فيمرّ');
  check(vowelledWasl('الْمَسِ اِثْنَيْنْ'),
    'وتُمسَك حيث وقعت من الجملة لا في صدرها وحدَه');
