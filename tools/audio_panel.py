...
تقيس المدد والبصمات والأحجام ولا **تسمع** الحروف. فالأذنُ هي الكاشفُ الوحيد، ولوحةٌ
تُسمِع بالتتابع وتبلّغ بضغطة تجعل مسحَ الدفعة عملاً لا نيّة.

**ولا تقرّ هذه اللوحةُ شيئاً**: تعرض وتسجّل ما سُمع وما بُلِّغ عنه، والحكمُ للأذن
البشرية وحدَها. والبلاغاتُ تُنسخ JSON ويطبّقها المصرِّف:

    python3 tools/generate_audio.py --requeue "<نص>,<نص>" --requeue-reason "بلاغ سماع"

**وحكمُ الأذن يُقيَّد بياناً** فلا يتكرّر السؤالُ على المالك ولا يُنسى جوابُه:

    python3 tools/generate_audio.py --verdict "خَمْسَةْ=طولُها طبيعيٌّ لا تكرار"
"""
...
GROUP_AR = dict(gen.CATEGORY_AR)          # الفئاتُ الأربع من المولّد نفسِه لا مكتوبةً هنا


def rows(since: float = 0.0) -> list:
    """صفوفُ اللوحة: مفتاحٌ ونصٌّ وفئةٌ ومدةٌ ومصدرُ طلبٍ — لكل ملفٍ في الفهرس."""
    ...
    for key, text in man.items():
        ...
        out.append({
            ...
            "src": gen.VOICE,
            "by": e.get("requestedBy", "—"),
            "verdict": verdicts.get(text, {}).get("verdict", ""),
        })
    order = gen.CATEGORY_ORDER
...
<title>لوحة فحص أصوات «اِحْسِبْ»</title>
<style>
 :root { --ink:#1d2733; --paper:#f7f9fb; --line:#cfdae4; --tint:#e3f0f3; --green:#2f7d6b; }
 ...
 button { font-family:inherit; cursor:pointer; border:1px solid var(--line);
          border-radius:.5rem; background:#fff; padding:.35rem .8rem; font-size:.95rem }
 button.tab.on { background:var(--tint); font-weight:700 }
 button small { display:block; font-size:.62rem; color:#6f8494; font-family:system-ui }
 ...
 .row { display:grid; grid-template-columns:2.6rem 1fr 5rem 8rem 5.5rem; align-items:center;
        gap:.6rem; padding:.4rem .5rem; border-bottom:1px solid #e6edf2 }
 .row:hover { background:#fff }
 .row.playing { background:#e8f4f0 }
 .row.done { opacity:.55 }
 .row.bad { background:#fbeee8 }
 .t { font-size:1.15rem; line-height:1.8 }
 .say { font-size:.72rem; color:#a06a4a; margin-inline-start:.6rem; font-family:system-ui }
 .prog { height:4px; background:#dde7ee; border-radius:2px; overflow:hidden; min-width:8rem }
 .prog i { display:block; height:100%; background:var(--green); width:0 }
 .meta { font-family:system-ui; font-size:.72rem; color:#6f8494 }
...
<footer>
  ...
  <span class="grow"></span>
  <span>الملفُّ يُشغَّل من <code>app/audio</code> — واللوحةُ لا تغيّر شيئاً</span>
</footer>
<script>
const DATA = PAYLOAD;
const KEY_DONE = 'ihsib.panel.done', KEY_BAD = 'ihsib.panel.bad';
...
function render() {
  ...
  $('#list').innerHTML = items.map((r) => `
    <div class="row ${done.has(r.key) ? 'done' : ''} ${bad.has(r.key) ? 'bad' : ''}" data-key="${r.key}">
      ...
      <div class="meta">${r.src} · ${r.by}${r.verdict ? ` · حُكم: ${r.verdict}` : ''}</div>
...
function playFrom(i) {
  ...
  play(items[i].key, () => setTimeout(() => playFrom(i + 1), 450));
}
...
render();
    ...
    html = (html.replace("TABS", tabs).replace("PAYLOAD", payload)
            .replace("TITLE", title or "لوحة فحص أصوات «اِحْسِبْ» — اسمع بنفسك"))
    out = PANEL / "index.html"
    out.write_text(html, encoding="utf-8")
    total = sum(r["sec"] for r in data)
    print(f"اللوحة: {out}  ({len(data)} ملفاً في {len(groups)} أقسام · "
          f"{total:.0f}ث سماعاً متتابعاً)")
    return out
...
def main() -> int:
    ap = argparse.ArgumentParser(description="لوحة فحص الأصوات بالأذن")
    ap.add_argument("--serve", action="store_true")
    ap.add_argument("--since", default="",
                    help="لا يُدرَج إلا ما كُتب بعد هذا الملف الشاهد (أو طابعٍ زمنيّ)")
