...
/* **والوسمُ يُقرأ ولا يُثبَّت** (تصحيحُ الجلسة ث٢): كان الرقمُ مكتوباً هنا (`v28`)
   فاحمرّ البابُ يومَ رُفعت القشرةُ إلى `v29` **بسببٍ لا صلةَ له بما يحرس** — والمحروسُ
   أن يكون الملفُّ في القشرة لا أن يكون رقمُها كذا. **وقد قِيس الأحمرُ على `main`
   نفسِه** (`git show HEAD` — قشرةُ `v29` وحارسٌ يطلب `v28`)، فرفعةُ قشرةٍ أخرى تعيده.
   فيُقرأ الوسمُ من مصدره ويُطبَع خبراً — والباقي محروسٌ كما كان. */
const sw = rootSrc('app/sw.js');
const shellTag = /const VERSION = '([^']+)'/.exec(sw)?.[1] || '';
ok(/'js\/placement\.js'/.test(sw) && Boolean(shellTag),
  `وهو في قشرة ${shellTag || 'بلا وسم'} فيعمل دون إنترنت`);
...
/* ٦) **ومعفاةٌ في المعاينة** (حكمُ المدير — الجلسة ط، تتمةُ بند د٣ المرفوع)
   **والعلّةُ بنيويّة تُقاس لا تُوعَد**: `save()` بلا أثرٍ على القرص حين `PREVIEW`، فلا
   تُكتب نجمةٌ ولا محاولة — **فـ`untouched()` صادقةٌ أبداً** مهما مشى المقيّمُ في
   الرحلة. فالشرطُ الذي يُخفي البطاقةَ عن طفلٍ بعد أوّل نجمة **لا يقع في المعاينة
   البتّة**: تظهر في كل لقطةِ خريطةٍ نولّدها وفي كل جولة قياس. */
const progSrc = src('progress.js');
ok(/if \(!PREVIEW\) \{[^\n]*\n\s*try \{\s*\n\s*localStorage\.setItem\(STORE_KEY/.test(progSrc),
  '**والمعاينةُ لا تكتب على القرص**: `save()` يتخطّى التخزينَ حين `PREVIEW` — فلا نجمةَ تبقى');
/* **فكلُّ فتحةِ معاينةٍ تبدأ من قرصٍ لم يُمَسّ** — والشرطُ يُعاد حسابُه بكراً في كل مرة.
   وهذه ليست حالةً نظرية: **جولاتُ القياس ولقطاتُ المراجعة تفتح بالمعاينة نفسِها**،
   فبلا هذا الإعفاء تحمل البطاقةُ نفسُها كلَّ صورةٍ نعرضها وكلَّ مقاسٍ نقيسه. */
const previewSurfaces = ['tools/browser_test.html', 'tools/browser_device.html']
  .filter((f) => /[?&]preview=1/.test(rootSrc(f)));
ok(previewSurfaces.length === 2,
  `وجولاتُ القياس تفتح بالمعاينة فعلاً (${previewSurfaces.length}/2) — فالإعفاءُ يبلغها`);
ok(/if \(progress\.PREVIEW\) return null;/.test(cardBlock),
  '**فتُعفى البطاقةُ في المعاينة** — تُرَدّ `null` قبل أن يُسأل حالُها أصلاً');
// **وقبل قراءة الحال لا بعدها**: إعفاءٌ بعده يقرأ التخزينَ ويحسب لا لشيء — والأهمُّ
// أنّ موضعَه يشهد أنّه قرارُ سياقٍ لا فرعٌ من فروع الحال الثلاثة.
ok(cardBlock.indexOf('progress.PREVIEW') < cardBlock.indexOf('firstRunState('),
  'وقبل حساب حالها — فهو قرارُ سياقٍ لا حالٌ رابعة');
// **وسنّةُ أخيه**: شريطُ التثبيت يُعفى في `main.js` بالشرط نفسِه — فالإعفاءان أخوان
// لا استثناءٌ يتيم (ومن رفع أحدهما وجد الآخر شاهداً على القاعدة).
ok(/if \(!progress\.PREVIEW\) install\.mount\(\);/.test(mainSrc),
  'وهي سنّةُ شريط التثبيت نفسِها في `main.js` — إعفاءان أخوان لا استثناءٌ يتيم');
