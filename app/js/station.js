...
export const SAY = {
  ...
  /* **وكشفُ القسمة بالعدل** (المرحلة ١٠ — الجلسة ث): «صَارَا سَوَاءْ» مثنّىً لا يصلح
     لثلاثة أوعية، فلها لفظُها الذي يصدق على الاثنين والثلاثة سواء. **والباقي يُقال
     حين يُرى**: يبقى في الكومة ظاهراً فيُسمّى — لا يُخفى ولا يُعَدّ خطأً
     (`METHOD.md §٣` المرحلة ١٠). وكلتاهما **قولُ ما يُرى** لا فعلٌ يُطلَب. */
  revealFair: 'صَارَ لِكُلٍّ مِثْلُ الْآخَرْ',
  revealLeft: 'وَبَقِيَ وَاحِدْ',
  bravo: 'أَحْسَنْتْ',
  great: 'رَائِعْ',
};
...
// ————— بوابةُ الجولة: **مالكُ القفل واحد** (بلاغُ الميدان ٦) —————
//
// **البلاغ**: «بعد اختيار الرمز الخطأ، لم يسمح بالعودة واختيار الرمز الصحيح» —
// البطاقةُ باقيةٌ محاطةً بالأحمر والشاشةُ لا تقبل نقرةً بعدها، والعهدُ صريحٌ في
// العهد (ب) أعلاه: **«ثم يحاول ثانية»**.
//
// **وعلّتُه بنيوية**: كانت كلُّ شاشةٍ تملك قفلَها (`let locked`) **وترفعه بسطرٍ في
// نهاية المسار السعيد** — وبين القفل وذلك السطر مخارجُ مبكرة (`if (!alive) return`)
// **وعطبٌ قد يُلقى**. وقد وقع الاثنان: مسارُ «أين هذا العدد؟» يَعُدّ **البطاقةَ التي
// لُمست**، وكانت بطاقةُ الاختيار تُرجِع نسخةً ناقصة من الشكل (بلا `ticks`) فتُلقي
// `countAloud` عطباً — فيُرفَض وعدُ المعالِج **ويبقى القفلُ مرفوعاً إلى الأبد**.
// (قِيس في المتصفّح: `TypeError: fig.ticks is not iterable` في `clearLine`.)
//
// **فالعلاجُ مالكٌ واحد**: القفلُ يُؤخَذ عند دخول المعالِج **ويُرَدّ في كل حال** —
// مخرجاً مبكراً كان أو عطباً يُلقى — إلا أن تُعلِن الجولةُ انتهاءها (`end`). ولا
// شاشةَ ترفع قفلَها بسطرٍ يُنسى في مسار.
//
// **والعطبُ يُقال ولا يُبتلَع**: يُطبَع في السجلّ باسم الشاشة، فمن قاس رأى — وحارسُ
// المتصفّح يقيس ما رأى الميدانُ: **خطأٌ ثم صوابٌ يُقبَل** في كل شاشة اختيار.

/**
 * قفلُ جولةٍ **يملكه هذا الملفّ وحدَه**.
 *
 * - `guard(fn)` يلفّ معالِجَ نقرة: يُهمِل النقرةَ ما دام مأخوذاً، ويأخذه للمعالِج،
 *   **ويردّه في كل مخرج** (عودةٌ عادية · مخرجٌ مبكر · عطبٌ يُلقى).
 * - `end()` تقولها الجولةُ التي **انتهت** (صوابٌ يمضي بالشاشة): يبقى مأخوذاً حتى
 *   تُستبدَل الشاشة، فلا نقرةَ تقع فوق احتفال.
 * - `held` حالُه — لأزرارٍ تُعطَّل بحال الجولة.
 * - `onFree` لوحٌ **يُعاد رسمُه كلّما رُدَّت الحرية** (الألواحُ التي تتبع أزرارُها حالَ
 *   الجولة — «اصنع العدد» وبناءُ الحزمة): فلا يبقى زرٌّ معطَّلاً لأنّ القفلَ كان
 *   مأخوذاً حين رُسِم آخرَ مرّة.
 */
export function roundGate(where = 'شاشة') {
  let held = false;
  let kept = false;
  let free = null;
  const fail = (err) => {
    // **ولا تُبتلَع**: عطبٌ في جولةٍ يُقال باسم شاشته، ثم تُردّ الحريةُ إلى الطفل
    console.error(`[station] عطبٌ في جولة «${where}» — القفلُ يُرَدّ`, err);
  };
  const release = () => { if (!kept) { held = false; free?.(); } };
  /** **والعطبُ يُرَدُّ معه القفلُ ولو أعلنت الجولةُ انتهاءها**: لا يُترَك طفلٌ أمام شاشةٍ صمّاء. */
  const releaseAfterFail = (err) => { fail(err); kept = false; release(); };
  return {
    get held() { return held; },
    set onFree(fn) { free = fn; },
    end() { kept = true; },
    guard(fn) {
      return (...args) => {
        ...
        try {
          ...
          releaseAfterFail(err);
          return undefined;
        }
        // معالِجٌ متزامن: تُردّ الحريةُ في اللحظة نفسِها (لا في دورةٍ لاحقة)
        ...
        return out.then(
          ...
          (err) => { releaseAfterFail(err); return undefined; },
        );
      };
    },
  };
...
function cardOf(spec, { onclick, label, className = '' }) {
  ...
  /* **والبطاقةُ شكلٌ تامّ كأخيها الصندوق** (بلاغ الميدان ٦): كانت تُرجِع **نسخةً
     مُنتقاةً** (`drawn` و`marks` و`plan`) — وهي **بطاقةٌ تُعَدّ أمام الطفل** في
     معالجة الخطأ («تُعَدّ البطاقةُ التي لمس»، عهدُ `numerals.js` الرابع)، فتطلب
     منها `countAloud` علاماتِ الخطّ وخاناتِ الإطار فلا تجدها **فتُلقي عطباً**
     يقتل الجولة. فصار الجردُ **واحداً في موضعٍ واحد** (`figureOf`) لا انتقاءً
     يفوته ما يُزاد فيه — كما هي `figureBox` سواءً بسواء. */
  return { ...fig, btn };
}
