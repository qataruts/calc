...
export function stationScreen({ nodeId, title, accent, make, view, score, save }) {
  ...
  function watchStep(model, api) {
    ...
    const run = async () => {
      ...
      act.replaceChildren();
      act.className = '';
      /* **ونمذجةٌ تُرتّب مشهدَها إن أعلنت ذلك** (`model.scene` — الجلسة م٩، بلاغ
         الميدان ٧): درسُ العبور **مشهدان لا مشهد** — إطارٌ ناقصٌ وحزمةٌ تعبر إليه، ثم
         الإطاران — والثاني لا يُرى قبل أن يقع العبور. وأشكالُ النمذجة **كلُّها مُعلَنة**
         فيجردها الحارس، **وترتيبُ ظهورها على مَن يملك الدرس** لا على الموجِّه: يُنادى
         قبل أوّل كلمة (فلا يلمح الطفلُ ما لم يحن وقتُه) **ومع كل إعادة** (فتُصفَّر
         آثارُ المشهد السابق). */
      model.scene?.(figures);
