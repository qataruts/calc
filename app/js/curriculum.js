 ...
 *   `units`       شريطٌ تُرصّ تحته وحداتٌ متلاصقة (المرحلة ١١ — القياسُ بوحدة)
 */
export const DISPLAYS = [
  'dice', 'scatter', 'objects', 'five-frame', 'ten-frame', 'two-frames',
  'numeral', 'line', 'pattern', 'scene', 'shape', 'units',
];
...
// ————— القِسمةُ بالتساوي: أوعيةٌ ومجاميعُ بيانَ منهجٍ (المرحلة ١٠ — الجلسة ث) —————
//
// **القسمةُ هي المزدوجُ مقلوباً** (`METHOD.md §٣` المرحلة ١٠): الجسورُ تشقّ الكلَّ
// شقّين، والمزدوجاتُ تجمع نصفين متساويين — وهذه تسأل: ثمانيةٌ بالعدل بين اثنين، فكم
// لكلٍّ؟ **ومجالٌ لا مدى**: لا عددَ فوق العشرين (ق٣ لم يُمَسّ بحرف).
//
// **والمجاميعُ بيانُ منهجٍ لا قرعةُ شاشة**: نصُّ المنهج يسمّيها لكل محطة (٤ · ٦ · ٨ ·
// ١٠ بين اثنين · ٥ على اثنين يبقى واحد · ٦ و٩ على ثلاثة)، فتُكتب هنا **بموضعها**
// (`at` — جزءُ محطتها كـ`SHAPES`) ويقرأها المولّد. ولو كُتبت في الشاشة لَجاز أن
// يتبدّل درسُ محطةٍ بلا أن يعلم المنهجُ — ولا يمسكه حارس.
//
// **وثلاثةُ شروطٍ يحرسها `check_range.py`** (بابٌ ١هـ): لكلِّ مجموعٍ **وعاءان فأكثر**،
// ولا مجموعَ فوق جبهة محطته، **وباقي القسمة يوافق موضعَه**: محطةُ «ما ينقسم وما لا
// ينقسم» **كلُّ مجاميعها تُبقي بقيّة**، وأخواتُها **لا تُبقي شيئاً** — فلا يلقى الطفلُ
// باقياً قبل درسه، ولا تخلو محطتُه منه.

export const SHARE = [
  { at: 'two', bowls: 2, totals: [4, 6, 8, 10], rest: false },
  { at: 'each', bowls: 2, totals: [4, 6, 8, 10], rest: false },
  { at: 'rest', bowls: 2, totals: [5, 7, 9], rest: true },
  { at: 'three', bowls: 3, totals: [6, 9], rest: false },
];

/** قِسمةُ محطةٍ بجزئها — أوعيتُها ومجاميعُها، أو `null` لموضعٍ لا قسمةَ فيه. */
export const shareAt = (part) => SHARE.find((s) => s.at === part) || null;

// ————— القياسُ بوحدة: طولُ الشريط بالوحدات (المرحلة ١١ — الجلسة ث) —————
//
// **الجسرُ من الوصف إلى القياس** (`METHOD.md §٣` المرحلة ١١): المرحلة ٨ تقارن
// «أطول/أقصر» بالعين، وهذه تسأل «كم وحدةً؟» — **وهو عدٌّ مُطبَّق**: تكرارُ وحدةٍ
// واحدة وعدُّها. ومدى طول الشريط بيانُ منهجٍ: **دونَ الاثنين لا قياس** (وحدةٌ واحدة
// لا تُري تكراراً)، **وفوق الثمانية** يصير العدُّ همَّ الشاشة لا القياسَ درسَها.
//
// ولا يخرج عن ق٣ بحال: سقفُه دون العشرين، ويقابله `check_range.py` بجبهة محطته.
export const UNIT_SPAN = { min: 2, max: 8 };

/**
 * **مفتاحُ ليتنر للقياس بوحدة — مدَاه مُشتقٌّ من الشريط لا مكتوبٌ بيد** (حكمُ المدير،
 * ١٣ أغسطس ٢٠٢٦؛ كان `measure|20|units`): **المدى في المفتاح يصف ما يُقاس فعلاً**،
 * وأطولُ شريطٍ ثمانِ وحدات — **ولوحةُ وليّ الأمر تقرؤه** فتقول «يقيس حتى…»، فمدىً
 * أوسعُ من الشريط يكذب على والد. وباشتقاقه من `UNIT_SPAN` لا يفترق الاثنان يوماً.
 */
export const UNIT_SKILL = `measure|${UNIT_SPAN.max}|units`;

// ————— الترتيبيّة: أسماءُ المواضع لا أعدادُها (٨·٥ — الجلسة ث) —————
//
// نصُّ `METHOD.md §٣`: «بعد (قبلُ وبعد) الزمنيّة، **موضعٌ يُسمّى بترتيبه** في صفٍّ من
// ثلاثة إلى خمسة، **ولغتُها أسماءُ الترتيب لا الأعداد**».
// فالجوابُ «الثَّالِثْ» لا «ثَلَاثَةْ» — والفرقُ درسٌ لا لفظ: العددُ يُخبر **كم**،
// والترتيبيُّ يُخبر **أين**.
//
// **والأولُ من اليمين** (واجهةٌ RTL): جهةُ الإطار الذي يُملأ من اليمين وخطِّ الأعداد
// الذي يبدأ منه — فلا يقلب الطفلُ جهتَه بين شاشتين. ويرسم المصيِّرُ الصفَّ من اليمين
// أصلاً (شريطُ النمط وصفُّ الأشكال ومشهدُ القياس كلُّها كذلك)، فالموضعُ **مقروءٌ من
// الرسم** لا محسوبٌ في الشاشة.
//
// **والأسماءُ تُنطَق** (كأسماء الأشكال): مشكولةٌ بالكامل، ومصفوفةٌ في القائمة الصوتية
// عبر `SPOKEN` في `patterns.js` — ومَن كتب المفتاحَ يسمّي مواضعه.
export const ORDINALS = [
  'الْأَوَّلْ', 'الثَّانِي', 'الثَّالِثْ', 'الرَّابِعْ', 'الْخَامِسْ',
];

/** أقصرُ صفٍّ ترتيبيّ — دونه لا «ثالث» يُسأل عنه (`METHOD.md §٣`: من ثلاثة إلى خمسة). */
...
/** أقسامُ اللوحة بترتيب الرحلة — سبعتُها الأُوَل كما في `METHOD.md §٦`. */
export const CONCEPT_SECTIONS = [
  ...
  // **وقسمُ القياس يفارق الأنماط** (الجلسة ث): كان `measure` ساكناً قسمَ «الأنماط»
  // لأنّ محطاته جاءت في مرحلتها، ثم دخلت **المرحلة ١١ (القياسُ بوحدة)** فصار
  // للمفهوم وجهان — وصفيٌّ بالعين وعدديٌّ بالوحدة — وكلاهما قياس. فأُفرد بقسمه
  // باسمه: الوالدُ يقرأ «القياس» حيث يبحث عنه، لا في ذيل الأنماط.
  { id: 'measure', title: 'القياس' },
  // **قسمٌ بالمرحلة ٩** (الجلسة ش): الهندسةُ مسارٌ موازٍ في مسارات التعلّم،
  // فلها في لوحة الوالد سطرُها — **مفهومٌ بعملية لا درجة** كإخوته.
  { id: 'shapes', title: 'الأشكال' },
  // **وقسمُ القسمة بالمرحلة ١٠** (الجلسة ث): القسمةُ بالعدل مفهومٌ قائمٌ بنفسه —
  // المزدوجُ مقلوباً — فلا تُدسّ في «العمليات» وهي بلا رمزِ عمليةٍ أصلاً.
  { id: 'share', title: 'القسمة' },
];
 ...
 *
 * **و`numeric` لمفهومٍ مدياتُه وصفيةٌ ثم دخله عددٌ** (الجلسة ث): مفتاحُ القياس
 * `measure|length|pick` وجهٌ يُسمّى، و`measure|8|units` **عددُ وحدات** — فلو قُرئ
 * العددُ خاماً لَقال السطرُ «يقارن ويقيس ٨» وهو كلامٌ لا يفهمه والد. فمن أعلن
 * `numeric` سُمّي مدَاه العدديُّ بعبارته هو، ومن لم يُعلنه بقي العددُ رقماً كما كان.
 */
export const CONCEPTS = {
  ...
  /* **والترتيبيّةُ مفهومٌ قائمٌ بنفسه لا وجهٌ من الترتيب** (الجلسة م٨، بند ب‑٦):
     كانت تحمل مفتاحَ `order` نفسَه (`order|5|rank`) وعبارتُه واحدة، **فكانت اللوحةُ
     تخترع في موضعٍ واحد**: عند الإتقان تختفي الترتيبيّةُ خلف «يرتّب حتى ١٠» فلا يعلم
     الوالدُ أنّ طفله يعرف الأوّلَ والثاني، وعند التعثّر فيها يقرأ «يحتاج تثبيت الترتيب
     حتى ٥» **وطفلُه يرتّب عشرةً بلا خطأ**، وفي الحال الوسطى تُقرأ الجملةُ **نكوصاً**.
     ومفتاحُها اليوم `rank|5|rank` — الترتيبُ يُخبر بالأكبر والأصغر، والترتيبيّةُ
     تُخبر بالموضع، وهما درسان في مرحلتين. */
  rank: {
    section: 'compare', title: 'الترتيبيّة: الأوّل والثاني',
    does: 'يعرف الأوّلَ والثاني والثالث في صفٍّ حتى {ما} مواضع',
    needs: 'يحتاج تثبيت الأوّل والثاني والثالث في صفٍّ حتى {ما} مواضع',
  },
  neighbor: {
  ...
  measure: {
    section: 'measure', title: 'القياس',
    does: 'يقارن ويقيس {ما}', needs: 'يحتاج تثبيت {ما}',
    // المدى العدديّ هنا **عددُ وحدات** لا عددٌ يُقاس عليه (المرحلة ١١)
    numeric: 'بالوحدات حتى {ما}',
  },
  share: {
    section: 'share', title: 'القسمة بالعدل',
    does: 'يقسم بالعدل حتى {ما}', needs: 'يحتاج تثبيت القسمة بالعدل حتى {ما}',
  },
  shape: {
...
export const STAGES = [
  ...
  },
  {
    ...
    stations: [
      {
        type: 'count', part: 'three', title: 'الْمِسْ وَعُدَّ ١–٣',
        frontier: { min: 1, max: 3, numeral: null, ops: [], signs: [], displays: ['dice', 'scatter', 'objects'] },
        skills: ['count|3|touch'],
      },
      {
        type: 'count', part: 'five', title: 'الْمِسْ وَعُدَّ ١–٥',
      ...
      {
        type: 'count', part: 'seven', title: 'الْمِسْ وَعُدَّ ٦–٧',
        frontier: { min: 1, max: 7, numeral: null, ops: [], signs: [], displays: ['scatter', 'objects'] },
        skills: ['count|7|touch'],
      },
      {
        type: 'count', part: 'ten', title: 'الْمِسْ وَعُدَّ ٨–١٠',
  ...
  },
  {
    ...
    stations: [
      ...
      {
        // **الترتيبيّة بعد «قبلُ وبعد»** (`METHOD.md §٣` — تعديلُ حزمة الإثراء):
        // الزمنُ يرتّب المشاهدَ، وهذه ترتّب **المواضع**: صفٌّ من ثلاثةٍ إلى خمسة،
        // والمطلوبُ موضعٌ **يُسمّى بترتيبه** لا بعدده. ومدَاها خمسةٌ (أطولُ صفٍّ)،
        // ولا رمزَ عدديّ فيها البتّة: لغتُها أسماءُ الترتيب.
        // **وأقصى رمزٍ خمسةٌ سقفاً لا درساً**: لا رقمَ يُعرَض في هذه المحطة البتّة
        // (لغتُها أسماءُ الترتيب)، والجبهةُ حدٌّ لِما يجوز لا وعدٌ بما يقع.
        type: 'rank', part: 'five', title: 'الأَوَّلُ وَالثَّانِي وَالثَّالِث',
        frontier: { min: 0, max: 5, numeral: 5, ops: [], signs: [], displays: ['objects', 'pattern'] },
        skills: ['rank|5|rank'],
      },
      {
  ...
  {
    // ————— المرحلة ١٠ — القِسمةُ بالتساوي (تعديلٌ معتمد، `METHOD.md §٣`) —————
    //
    // **أعمقُ المؤجَّلات وأوفقُها لِما بُني**: الجسورُ تشقّ الكلَّ شقّين (المرحلة ٥)،
    // والمزدوجاتُ تجمع نصفين متساويين (٦·٥ و٧·٩) — **والقسمةُ بالتساوي هي المزدوجُ
    // مقلوباً**: ثمانيةٌ تُقسَم بالعدل فيصير لكلٍّ أربعة. وطفلُ الخامسة يقسم بالعدل
    // فطرةً قبل أن يعرف اسمها، فالمحطةُ تسمّي ما يفعله لا تعلّمه فعلاً جديداً.
    //
    // **ولا رمزَ عمليةٍ ولا عملية**: `ops: []` و`signs: []` — القسمةُ **تُصنَع باللمس**
    // كـ«اجعلهما سواء»، ورمزُها بابُ مستوىً آخر. **ومجالٌ لا مدى**: أقصاها العشرة.
    id: 'stage10',
    title: 'القِسْمَةُ بِالعَدْل',
    sub: 'يُوَزِّعُ الكَمِّيَّةَ بِالتَّسَاوِي، وَيَرَى مَا بَقِيَ فَلَا يُخْفِيه',
    accent: BOND_ACCENT,
    kind: 'bond',
    mark: 'share',
    stations: [
      {
        // **العدلُ يُصنَع باللمس**: كميةٌ تُوزَّع على وعاءين حتى يستويا ولا يبقى
        // ما يُقسَم — والحكمُ **كبسةٌ يُعلنها الطفل** (عهدُ م٦)، لا وقوعٌ صدفة.
        type: 'share', part: 'two', title: 'بِالعَدْلِ بَيْنَ اثْنَيْن',
        frontier: { min: 0, max: 10, numeral: 10, ops: [], signs: [], displays: ['objects', 'scatter'] },
        skills: ['share|10|make'],
      },
      {
        // **الجوابُ حصّةٌ لا مجموع**: يُرى الكلُّ مقسوماً ويُسأل عن **نصيب الواحد** —
        // والمشتّتُ الأولُ **المجموعُ نفسُه**، فهو عينُ الالتباس الذي تُبنى له المحطة.
        type: 'share', part: 'each', title: 'كَمْ لِكُلِّ وَاحِد؟',
        frontier: { min: 0, max: 10, numeral: 10, ops: [], signs: [], displays: ['objects', 'scatter', 'numeral'] },
        skills: ['share|10|each'],
      },
      {
        // **البقيّةُ تُرى ولا تُخفى** (`METHOD.md §٣`): خمسةٌ على اثنين يبقى واحد —
        // يبقى ظاهراً خارج الوعاءين ويُسمّى، **ولا يُعَدّ خطأً**: تمهيدٌ صادقٌ للقسمة
        // وعهدُ «لا شاشة خطأ» معاً.
        type: 'share', part: 'rest', title: 'مَا يَنْقَسِمُ وَمَا لَا يَنْقَسِم',
        frontier: { min: 0, max: 10, numeral: 10, ops: [], signs: [], displays: ['objects', 'scatter'] },
        skills: ['share|10|make'],
      },
      {
        // **القسمةُ تتجاوز النصف**: ثلاثةُ أوعية — فلا تُقرأ القسمةُ «نصفين» أبداً.
        type: 'share', part: 'three', title: 'بِالعَدْلِ بَيْنَ ثَلَاثَة',
        frontier: { min: 0, max: 10, numeral: 10, ops: [], signs: [], displays: ['objects', 'scatter'] },
        skills: ['share|10|make'],
      },
    ],
  ...
  {
    // ————— المرحلة ١١ — القياسُ بوحدة (تعديلٌ معتمد، `METHOD.md §٣`) —————
    //
    // **الجسرُ من الوصف إلى القياس**: المرحلة ٨ تقارن «أطولُ وأقصر» بالعين، وهذه
    // تسأل **«كم وحدةً؟»** — فتصير المقارنةُ قياساً، **وهو عدٌّ مُطبَّق** (تكرارُ
    // وحدةٍ واحدة وعدُّها). ومسارُ الطول عند Clements & Sarama يضع «القياسَ طرفاً
    // لطرف» في الخامسة، فموضعُها ذيلُ المستوى لا بابُ مستوىً آخر.
    //
    // **ولا فجوةَ ولا تراكب**: يفرضه المصيِّرُ بنيةً وحارسُه قياساً — فالقياسُ
    // الصادق **شرطُ بنيةٍ لا وعدُ رسم**. **والوحدةُ واحدةٌ في السؤال**.
    id: 'stage11',
    title: 'القِيَاسُ بِوَحْدَة',
    sub: 'كَمْ وَحْدَةً طُولُه؟ — وَالقِيَاسُ يَحْكُمُ حَيْثُ تَخْدَعُ العَيْن',
    accent: PATTERN_ACCENT,
    kind: 'pattern',
    mark: 'ruler',
    stations: [
      {
        // **الوحدةُ تُكرَّر ولا تُترَك فجوة**: شريطٌ مرصوصٌ تحته وحدات، والجوابُ
        // **عددُ ما رُصّ مقروءاً من الرسم** لا من طولٍ يُحسَب.
        type: 'units', part: 'count', title: 'كَمْ وَحْدَةً طُولُه؟',
        frontier: { min: 0, max: 20, numeral: 20, ops: [], signs: [], displays: ['units', 'numeral'] },
        skills: [UNIT_SKILL],
      },
      {
        // **القياسُ يحكم حيث تخدع العين**: شريطان يفترقان بوحدةٍ واحدة — لا تحسمها
        // النظرةُ وتحسمها العُدّة.
        type: 'units', part: 'longer', title: 'أَيُّهُمَا أَطْوَلُ بِالوَحَدَات؟',
        frontier: { min: 0, max: 20, numeral: 20, ops: [], signs: [], displays: ['units', 'numeral'] },
        skills: [UNIT_SKILL],
      },
      {
        // **من القراءة إلى الصنع**: يرصّ الطفلُ الوحداتِ بنفسه حتى يستوفي الشريط،
        // **ويُعلن قياسَه كبساً**، ثم تُعَدّ أمامه — فالعدديّةُ ختامُ القياس.
        type: 'units', part: 'hand', title: 'قِسْ بِيَدِك',
        frontier: { min: 0, max: 20, numeral: 20, ops: [], signs: [], displays: ['units', 'numeral'] },
        skills: [UNIT_SKILL],
      },
    ],
...
export const GATES = [
  ...
  },
  {
    ...
    // **وتَبعت الرحلةَ ثانيةً حين طالت** (الجلسة ث): دخلت **المرحلتان ١٠ و١١** في
    // الذيل، فصارت بعدهما — نصُّ `METHOD.md §٣`: بوابةُ الختام آخرُ الرحلة. ومدَاها
    // `stage1` كما هو، فتشمل القسمةَ والقياسَ بالوحدة من تلقائها.
    id: 'final',
    after: 'stage11',
    from: 'stage1',
